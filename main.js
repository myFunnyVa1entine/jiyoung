...
const swipeDelta = (dx, dy) =>
  Math.abs(dx) > Math.abs(dy) && Math.abs(dx) > 36 ? (dx < 0 ? 1 : -1) : 0;
const wheelDelta = (e) =>
...
const moveTo = (s, d, total) => ({
  cur: nextIndex(s.cur, d, total),
  busy: true,
});

// ── 원자 DOM 조작 (사이드이펙트) ──────────────────
const setTransform = (el, i) => {
  el.style.transform = toTransform(i);
};
const setWidth = (el, v) => {
  el.style.width = v;
};
...
const triggerAnim = (el, delay) =>
  setTimeout(() => setClass(el, "in", true), delay);
...
document.addEventListener("click", (e) => {
  // 버튼, 링크, 도트 클릭은 제외
  if (e.target.closest(".arr, .dot, .btn-call, a")) return;
